...
DATA_PATHS = [
    Path("~/IsaacLab_5/g1_webxr_left_calibration_300.json").expanduser(),
]

OUT_PATH = Path("~/IsaacLab_5/left_wrist_mapping_model_300.pt").expanduser()
...
def build_feature(sample):
    """
    Same feature style as right hand:
    ...
    wrist_q = q_normalize(sample["webxr_left_wrist_quat_isaac_wxyz"])
    palm_q = q_normalize(sample["webxr_left_palm_quat_isaac_wxyz"])
    palm_n = torch.tensor(sample["webxr_left_palm_normal_isaac_xyz"], dtype=torch.float32)

    feats.append(wrist_q)
    feats.append(palm_q)
    feats.append(palm_n)

    kps = sample["webxr_left_keypoints"]
...
def build_target(sample):
    return torch.tensor(
        sample["robot_left_wrist_joint_values_roll_pitch_yaw"],
        dtype=torch.float32,
    )
...
def load_all_samples():
    ...
    for s in all_samples:
        if s.get("webxr_left_wrist_quat_isaac_wxyz") is None:
            continue
        if s.get("webxr_left_palm_quat_isaac_wxyz") is None:
            continue
        if s.get("webxr_left_palm_normal_isaac_xyz") is None:
            continue
        if len(s.get("webxr_left_keypoints", {})) < 20:
            continue
